...
int decode0_JPEG(
    struct DLL_STRPICENV *env, int size, UCHAR *fp0, int b_type, UCHAR *buf, int skip
) {
  JPEG *jpeg = (JPEG *)(((int *)env) + 128);
  jpeg->fp = fp0;
  jpeg->fp1 = fp0 + size;
...
int jpeg_dqt(JPEG *jpeg) {
  ...
  if (jpeg->fp + 2 > jpeg->fp1) goto err;
  size = (jpeg->fp[0] << 8 | jpeg->fp[1]) - 2;
  jpeg->fp += 2;
  if (jpeg->fp + size > jpeg->fp1) goto err;

  while (size > 0) {
    c = *jpeg->fp++;
    size--;
    j = c & 7;
    if (j > jpeg->n_dqt) jpeg->n_dqt = j;
    ...
    if (c & 0xf8) {
      ...
      //  8 bit DQT
      for (i = 0; i < 64; i++) jpeg->dqt[j][i] = *jpeg->fp++;
      size -= 64;
    }
  }
  return 0;
...
int jpeg_dht(JPEG *jpeg) {
  ...
  while (len > 0) {
    if (jpeg->fp + 17 > jpeg->fp1) goto err;
    val = jpeg->fp[0];
    ...
    for (i = 1; i <= 16; i++) {
      Li[i] = jpeg->fp[i];
      num += Li[i];
      for (j = 0; j < Li[i]; j++) table->size[k++] = i;
    }
    table->elem = num;
    jpeg->fp += 17;
    ...
    i = table->size[0];
    while (k < num) {
      while (table->size[k] == i) table->code[k++] = code++;
      if (k >= num) break;
      do {
        code <<= 1;
        i++;
    ...
    if (jpeg->fp + num > jpeg->fp1) goto err;
    for (k = 0; k < num; k++) table->value[k] = jpeg->fp[k];
    jpeg->fp += num;
...
int jpeg_init(JPEG *jpeg) {
  ...
  for (;;) {
    if (jpeg->fp + 2 > jpeg->fp1) goto err;
    if (jpeg->fp[0] != 0xff) goto err0;
    c = jpeg->fp[1];
    jpeg->fp += 2;
    if (c == 0xd8) continue; /* SOI */
    if (c == 0xd9) goto err; /* EOI */

    if (c == 0xc0) jpeg_sof(jpeg);
    else if (c == 0xc4) jpeg_dht(jpeg);
    else if (c == 0xdb) jpeg_dqt(jpeg);
    else if (c == 0xdd) {
      if (jpeg->fp + 4 > jpeg->fp1) goto err;
      jpeg->interval = jpeg->fp[2] << 8 | jpeg->fp[3];
      jpeg->fp += 4;
    } else if (c == 0xda) {
      if (jpeg->fp + 3 > jpeg->fp1) goto err;
      jpeg->scan_count = jpeg->fp[2];
      jpeg->fp += 3;
      if (jpeg->fp + jpeg->scan_count * 2 > jpeg->fp1) goto err;
...
void jpeg_decode_init(JPEG *jpeg) {
  ...
  for (i = 0; i < 32 * 32 * 4; i++) jpeg->mcu_buf[i] = 0x80;

  for (i = 0; i < jpeg->scan_count; i++) jpeg->mcu_yuv[i] = jpeg->mcu_buf + (i << 10);
  return;
}
...
void jpeg_idct_init(int base_img[64][64]) {
  ...
  cost[14] = -30274;
  cost[15] = -32138;
  for (u = 0; u < 16; u++) cost[u + 16] = -cost[u];
...
int jpeg_get_value(JPEG *jpeg, int size) {
  ...
  if (size) {
    ...
    if (val < 0) val = 0x10000 | (0 - val);
    else if (!(val & (1 << (size - 1)))) val -= (1 << size) - 1;
  }
  return val;
}
...
int jpeg_decode_huff(JPEG *jpeg, int scan, int *block, UCHAR *zigzag_table) {
  ...
  while (index < 64) {
    ...
    // ZRL
    while (run-- > 0) block[zigzag_table[index++]] = 0;

    block[zigzag_table[index]] = val * pQt[index];
    index++;
  }
  while (index < 64) block[zigzag_table[index++]] = 0;
  return 0;
}
...
void jpeg_mcu_bitblt(int *src, int *dest, int width, int x0, int y0, int x1, int y1) {
  ...
  for (y = 0; y < h; y++) {
    y2 = (y * 8 / h) * 8;
    for (x = 0; x < w; x++) *dest++ = src[y2 + (x * 8 / w)];
    dest += width;
  }
}
...
int jpeg_decode_yuv(JPEG *jpeg, int h, int v, unsigned char *rgb, int b_type) {
  ...
  for (y = 0; y < y1; y++) {
    for (x = 0; x < x1; x++) {
      ...
      if (b & 0xffffff00) b = (~b) >> 24;

      /* green */
      g = 128 + ((Y12 - V * 0x0B6C) >> 12);
      if (g & 0xffffff00) g = (~g) >> 24;

      /* red */
      r = 128 + ((Y12 + V * 0x166E) >> 12);
      if (r & 0xffffff00) r = (~r) >> 24;
      if (b_type == 0x0004) {
        rgb[0] = b;
        rgb[1] = g;
        rgb[2] = r;
