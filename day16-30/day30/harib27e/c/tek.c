...
int tek_getsize(unsigned char *p) {
  ...
  int size = -1;
  if (my_memcmp(p + 1, header, 15) == 0 && (*p == 0x83 || *p == 0x85 || *p == 0x89)) {
    p += 16;
...
static int tek_lzrestore_stk1(int srcsiz, UCHAR *src, int outsiz, UCHAR *q) {
  ...
  do {
    if ((by = (lz = *s7ptr++) & 0x0f) == 0) by = tek_getnum_s7s(&s7ptr);
    if ((lz >>= 4) == 0) lz = tek_getnum_s7s(&s7ptr);
    do {
      *q++ = *s7ptr++;
    } while (--by);
    if (q >= q1) {
  ...
  if (outsiz) {
    ...
    do {
      ...
      do {
        ...
        cp = i >> 4;
        i &= 0x0f;
        if ((i & 1) == 0) i |= (tek_getnum_s7(&s7ptr) + 1) << 4;
        i >>= 1;
        ds = ~(i - 6);
        if (i < 4) ds = repdis[i];
        if (i == 4) ds = repdis[0] - tek_getnum_s7(&s7ptr) - 1;
        if (i == 5) ds = repdis[0] + tek_getnum_s7(&s7ptr) + 1;
        if (cp == 0) cp = tek_getnum_s7(&s7ptr) + 16;
        cp++;
        if (i > 0) {
          if (i > 1) {
...
static int tek_decmain5(
    int *work, UCHAR *src, int osiz, UCHAR *q, int lc, int pb, int lp, int flags
);
...
static int tek_lzrestore_tek5(int srcsiz, UCHAR *src, int outsiz, UCHAR *outbuf) {
  ...
  if ((fl = (prop0 = *src) & 0x0f) == 0x01) /* 0001 */
    flags |= -1;
  else if (fl == 0x05) flags = -2;
  else if (fl == 0x09) flags &= 0;
  else return 1;
  src++;
  prop0 >>= 4;
  if (prop0 == 0) prop0 = *src++;
  else {
...
static int tek_rdget1(
    struct tek_STR_RNGDEC *rd, tek_TPRB *prob0, int n, int j, struct tek_STR_BITMODEL *bm
) {
...
static int tek_decmain5(
  ...
  for (i = 0; i < 4; i++) rep[i] = ~i;
  if (__builtin_setjmp(rd->errjmp)) goto err;
  ...
  if (stk) {
    rd->rmsk = -1 << 11;
    for (i = 0; i < 32; i++) rd->bm[i].lt = 0;
    for (i = 0; i < 14; i++) rd->ptbm[i] = &rd->bm[0];
  } else {
    UCHAR pt[14];
    static UCHAR pt1[14] = {8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 18, 18, 18, 8};
    static UCHAR pt2[14] = {8, 8, 10, 11, 12, 12, 14, 15, 16, 16, 18, 18, 20, 21};
    /*
    ...
    */
    ...
    if (flags == -2) { /* z1 */
      rd->bm[22].lt = 0;
      for (i = 0; i < 14; i++) pt[i] = pt1[i];
    } else {
      for (i = 0; i < 14; i++) pt[i] = pt2[i];
    ...
    for (i = 0; i < 14; i++) rd->ptbm[i] = &rd->bm[pt[i]];
  }
  for (i = 0; i < 32; i++) tek_setbm5(&rd->bm[i], rd->bm[i].t, rd->bm[i].m);

  if ((tek_rdget1(rd, &prb->pb[0].st[0].mch, 0x71, 0, rd->ptbm[0]) ^ stk) == 0) goto err;
  *q++ = tek_rdget1(rd, prb->lit, lit0cntmsk, 1, &rd->bm[24]) & 0xff;
  ...
  while (pos < osiz) {
    ...
    if (tek_rdget1(rd, &prb->pb[s_pos].st[s].mch, 0x71, 0, rd->ptbm[s > 0]) ^ stk) {
      ...
      s = state_table[s];
      if (pmch == 0) *q = tek_rdget1(rd, &prb->lit[i], lit0cntmsk, 1, &rd->bm[24]) & 0xff;
      else {
      ...
      if (tek_rdget1(rd, &prb->st[s].rep, 0x71, 0, rd->ptbm[13]) ^ stk) { /* len/dis */
        ...
        if (j >= 4) {
          ...
          if (j < 14) /* k < 6 */
            rep[0] |= tek_revbit(
                tek_rdget1(
                    rd, &prb->spdis[j & 1][(1 << k) - 2], k | 0x70, 1, rd->ptbm[10 + (k >= 4)]
                ),
                k
            );
          else {
            if (stk == 0) {
              if (k -= 6) rep[0] |= tek_rdget0(rd, k, ~0) << 6;
        ...
        if (tek_rdget1(rd, &prb->st[s].repg0, 0x71, 0, rd->ptbm[13]) ^ stk) { /* rep0 */
          ...
          if (tek_rdget1(rd, &prb->st[s].repg1, 0x71, 0, rd->ptbm[13]) ^ stk) /* rep1 */
            ...
            if (tek_rdget1(rd, &prb->st[s].repg2, 0x71, 0, rd->ptbm[13]) ^ stk) /* rep2 */
              ...
              if (stk == 0) {
                if (tek_rdget1(rd, &prb->repg3, 0x71, 0, &rd->bm[22]) == 0) goto err;
              }
              i = rep[3]; /* rep3 */
              rep[3] = rep[2];
            }
    ...
    skip:
      i += 2;
      if (pos + rep[0] < 0) goto err;
      if (pos + i > osiz) i = osiz - pos;
      pos += i;
      do {
        *q = q[rep[0]];
        q++;
...
static int tek_decode5(int siz, UCHAR *p, UCHAR *q) {
  ...
  if ((dsiz = tek_getnum_s7s(&p)) > 0) {
    hed = tek_getnum_s7s(&p);
    if ((hed & 1) == 0) st = tek_lzrestore_tek5(p1 - p + 1, p - 1, dsiz, q);
    else {
      bsiz = 1 << (((hed >> 1) & 0x0f) + 8);
      if (hed & 0x20) return 1;
      if (bsiz == 256) st = tek_lzrestore_tek5(p1 - p, p, dsiz, q);
      else {
        if (dsiz > bsiz) return 1;
        if (hed & 0x40) tek_getnum_s7s(&p);
